...
Function check
	...
	var $width2; $width3; $width4; $width5 : Integer
...
Function setStatus($status : Integer)
...
Function render($x : Integer; $y : Integer; $lastSize : Pointer)->$ob : Object
	// render a button object (x; y; pointer to boolean last displayed size) -> object for json form
	
	Case of 
		: (This:C1470.mytype=0)
			$ob:=This:C1470.renderButton($x; $y; $lastSize)
		: (This:C1470.mytype=1)
			$ob:=This:C1470.renderSubform($x; $y; $lastSize)
		Else 
			$ob:=New object:C1471
	End case 
	
	
Function renderSubform($x : Integer; $y : Integer; $lastSize : Pointer)->$buttonobject : Object
	// render a subform, called by render()
	
	var $name : Text:=String:C10(This:C1470.name)
	var $buttonwidth : Integer:=Num:C11(This:C1470.curWidth)
...
Function renderButton($x : Integer; $y : Integer; $lastSize : Pointer)->$buttonobject : Object
	// render a button, called by render()
	
	var $button : Object:=This:C1470
	var $name : Text:=String:C10($button.name)
	var $buttonwidth : Integer:=Num:C11($button.curWidth)
	...
	End if 
	If ($button.fontSize#Null:C1517)
		$buttonobject.fontSize:=$button.fontSize
	End if 
