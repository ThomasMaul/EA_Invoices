...
Function loadEvent($that : Object)
	// function is called via Execute method in subform. For such calls, This is not set, we need to pass it
	If (($that=Null:C1517) && (This:C1470#Null:C1517))
		$that:=This:C1470
	End if 
	
	If ($that.data#Null:C1517)
		$that.itemList:=$that.data.invoice_lines.orderBy("ItemPos")
		
		
		If ($that.data.ProForma)
		...
		Else 
			...
			OBJECT SET VISIBLE:C603(*; "inv_Payement@"; Bool:C1537($that.data.Paid))
		End if 
