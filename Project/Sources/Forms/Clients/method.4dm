...
Case of 
	: ($event=On Load:K2:1)
		...
		Form:C1466.wpLoaded:=False:C215
		
		
		//General
		Form:C1466.queryString:=""
		Form:C1466.inSelection:=False:C215
		Form:C1466.dataClass:=ds:C1482.CLIENTS
		Form:C1466.dataClassName:="CLIENTS"  //To be replaced by :=$dataClass.getInfo().name  //The Table (DataClass) name
		Form:C1466.dataClassPtr:=->[CLIENTS:1]  //To be replaced when it will be possible to get a pointer on a DataClass...
		Form:C1466.objectsNames:=New collection:C1472("Optional_Data"; "Numbers"; "eAddresses")  //Names ob Object Fields containing Multiple Values
		...
		For each ($listBox; Form:C1466.objectsNames)
			Settings_ApplyToListBox(Form:C1466.settings; "_LB_"+$listBox; True:C214)
		End for each 
	...
	: ($event=On Page Change:K2:54)
		
		If (Form:C1466.wpLoaded=False:C215)
			Form:C1466.wpLoaded:=True:C214
...
If (($event=On Load:K2:1) | ($event=On Selection Change:K2:29) | ($event=On Clicked:K2:4) | ($event=On Drop:K2:12) | ($event=On Data Change:K2:15) | ($event=On After Edit:K2:43))
	...
	$what:=OBJECT Get pointer:C1124(Object named:K67:5; "_MORE_")->
	OBJECT SET VISIBLE:C603(*; "@_1_@"; ($what=0))
	OBJECT SET VISIBLE:C603(*; "@_2_@"; ($what=1))
End if 
